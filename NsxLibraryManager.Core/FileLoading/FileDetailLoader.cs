...
public class FileDetailLoader
{
    private readonly IPackageTypeAnalyzer _packageTypeAnalyzer;
    private readonly IKeySetProviderService _keySetProviderService;
    
    public FileDetailLoader(IPackageTypeAnalyzer packageTypeAnalyzer, IKeySetProviderService keySetProviderService)
    {
    ...
    private void saveIcon(FileContents fileContents)
    {

        if (fileContents.Icon == null) return;
        var iconPath = @"N:/roms/testa/icon.jpg";
        File.WriteAllBytes(iconPath, fileContents.Icon);
